...
const productService = {
    getProducts: (): Product[] => {
        return [
            { id: 1, name: "iPhone 16 Pro Max 256GB", price: 44900, quantity: 0 },
            { id: 2, name: "iPhone 16 Pro 128GB", price: 36900, quantity: 0 },
            { id: 3, name: "iPhone 16 Plus 128GB", price: 32900, quantity: 0 },
            { id: 4, name: "iPhone 16 128GB", price: 29900, quantity: 0 },
            { id: 5, name: "iPhone 15 Pro Max", price: 45900, quantity: 0 },
            { id: 6, name: "iPhone 15 Pro", price: 36900, quantity: 0 },
        ];
    }
}
export default function ShoppingCart() {
    ...
    const generatePurchaseOrder = () => {
        ...
        products.forEach(product => {
            if (product.quantity > 0) {
                content += `${product.name}\t${product.quantity}\t${product.price.toLocaleString()}\t${(product.price * product.quantity).toLocaleString()}\n`;
            }
        });

        content += '----------------------------------------\n';
        content += `總金額：${total.toLocaleString()}`;
    ...
    <div>
        ...
        <Card className="min-h-screen w-full p-4 bg-gray-500">
            <Card title="進貨訂單" className="shadow-lg rounded-xl bg-slate-50">
                <div className="flex flex-col overflow-x-auto">
                    {/* 標題列 */}
                    <div className="flex flex-row items-center font-bold text-base border-b py-2 bg-gray-100">
                        <div className="flex-1 text-left pl-2">機型</div>
                    ...
                    {products.map((product) => (
                        ...
                        >
                            ...
                            <div className="flex-1 text-blue-600 text-center">${product.price.toLocaleString()}</div>
                            ...
                            <div className="flex-1 text-green-600 font-semibold text-center">
                                小計: ${ (product.price * product.quantity).toLocaleString()}
                            </div>
                        </div>
                    ))}
                    ...
                    <div className="flex flex-col items-center gap-4 mt-4">
                        ...
                        <div className="text-2xl font-bold text-green-600">總金額: ${total.toLocaleString()}</div>
